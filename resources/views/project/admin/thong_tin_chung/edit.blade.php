        ...
        <div class="row">
            <div class="col-xl-9 col-lg-10 col-md-12 col-sm-12 mx-auto">
                <div class="tm-bg-primary-dark tm-block tm-block-h-auto">
                    ...
                    <div>
                        <div>
                            <form action="{{route('update_thong_tin_chung',$thong_tin_chung->id)}}" method="post" class="tm-edit-product-form" enctype="multipart/form-data">
                                @csrf
                                @Method('PUT')
                                <div class="form-group mb-3">
                                    <label for="">Tiêu đề
                                    </label>
                                    <input id="tieu_de" name="tieu_de" value="{{ $thong_tin_chung->tieu_de }}" type="text" class="form-control validate" required />
                                </div>
                                <div class="form-group mb-3" style="margin-top:0px">
                                    <label for="">Nội dung</label>
                                    <textarea class="form-control" id="ckeditor" name="noi_dung" value="" rows="5">
                                        {!!$thong_tin_chung->noi_dung!!}
                                    </textarea>
                                </div>
                                <div class="form-group mb-3">
                                    <label for="category">Nhóm thông tin</label>
                                    <select class="custom-select tm-select-accounts" name="status" style="width:100%" id="">
                                        <option value="0" @if($thong_tin_chung->status == '0' ) selected @endif>Hiển thị</option>
                                        <option value="1" @if($thong_tin_chung->status == '1' ) selected @endif>Nội dung xác nhận</option>
                                        <option value="1" @if($thong_tin_chung->status == '2' ) selected @endif>Điều khoản thanh toán</option>
                                        <option value="1" @if($thong_tin_chung->status == '3' ) selected @endif>Tư vấn bán hàng</option>
                                        <option value="1" @if($thong_tin_chung->status == '4' ) selected @endif>Đơn hàng tối thiểu</option>
                                    </select>
                                </div>
                                <button type="submit" class="btn btn-primary btn-block text-uppercase">Edit Now</button>
                            </form>
                        </div>
                    </div>
                </div>
