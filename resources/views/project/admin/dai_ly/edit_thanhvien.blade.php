        ...
        <div class="row">
            <div class="col-xl-9 col-lg-10 col-md-12 col-sm-12 mx-auto">
                <div class="tm-bg-primary-dark tm-block tm-block-h-auto">
                    <div class="row">
                        <div class="col-12">
                            <h2 class="tm-block-title d-inline-block">Chỉnh sửa thông tin thành viên</h2>
                        </div>
                    </div>
                    <div class="row tm-edit-product-row">
                        <div class="col-xl-6 col-lg-6 col-md-12">
                            <form action="{{ route('update_thanh_vien',$thanh_vien->id) }}" method="post" enctype="multipart/form-data" class="tm-edit-product-form">
                                @csrf
                                @method('PUT')
                                <div class="form-group mb-3">
                                    <label for="name">Mã khách hàng
                                    </label>
                                    <input id="ma_khach_hang" name="ma_khach_hang" type="text" class="form-control validate" value="{{ $thanh_vien->ma_khach_hang }}" />
                                </div>
                                <div class="form-group mb-3">
                                    <label for="name">Tên
                                    </label>
                                    <input id="ten" name="ten" type="text" class="form-control validate" value="{{ $thanh_vien->ten  }}" required />
                                    @error('name')
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input id="ten_nha_thuoc" name="ten_nha_thuoc" type="text" class="form-control validate" value="{{ $thanh_vien->ten_nha_thuoc  }}" required />
                                    @error('name')
                                    <div class="alert alert-danger" id="nameError">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="form-group mb-3">
                                    ...
                                    <input id="email" name="email" type="text" class="form-control validate" value="{{ $thanh_vien->email  }}" />
                                    @error('email')
                                    <div class="alert alert-danger" id="nameError">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="form-group mb-3">
                                    ...
                                    <input id="so_dien_thoai" name="so_dien_thoai" type="number" class="form-control validate" value="{{ $thanh_vien->so_dien_thoai  }}" required />
                                    @error('sodienthoai')
                                    <div class="alert alert-danger" id="nameError">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="row">
                                    ...
                                    <div class="form-group mb-3 col-xs-12 col-sm-6">
                                        ...
                                        <select class="custom-select tm-select-accounts" name="id_tinh" style="width:100%" id="">
                                            @foreach ($tinh as $t)
                                            <option value="{{ $t->id }}" @if ($thanh_vien->id_tinh == $t->id) selected @endif>{{$t->ten }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group mb-3">
                                    ...
                                    <select id="trangthai" name="status" class="custom-select tm-select-accounts" style="width:100%" required>
                                        <option value="0" @if($thanh_vien->status == '0' ) selected @endif>Tạm thời</option>
                                        <option value="1" @if($thanh_vien->status == '1' ) selected @endif>Chờ xác thực</option>
                                        <option value="2" @if($thanh_vien->status == '2' ) selected @endif>Chính thức</option>
                                    </select>
                                    @error('trangthai')
                        ...
                        <div class="col-xl-6 col-lg-6 col-md-12 mx-auto mb-4">
                            <div class="tm-product-img-dummy mx-auto" id="previewImageContainer">
                                ...
                                <img src="{{ asset('upload/img_dai_ly/' . $thanh_vien->thumbnail) }}" alt="Product Image">
                            </div>
                            ...
                            <div class="form-group mb-3" style="margin-top: 90px;">
                                ...
                                <input id="dia_chi_nha_thuoc" name="dia_chi_nha_thuoc" type="text" class="form-control validate" value="{{ $thanh_vien->dia_chi_nha_thuoc  }}" required />
                                @error('sodienthoai')
                                <div class="alert alert-danger" id="nameError">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-group mb-3">
                                ...
                                <input id="ma_so_thue" name="ma_so_thue" type="text" class="form-control validate" value="{{ $thanh_vien->ma_so_thue  }}" />
                                @error('sodienthoai')
                                <div class="alert alert-danger" id="nameError">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group mb-3" style="margin-top:77px">
                                ...
                                <select class="custom-select tm-select-accounts" name="id_nguoi_quan_ly" style="width:100%" id="">
                                    ...
                                    <option value="{{ $dd->id }}" @if ($thanh_vien->id_nguoi_quan_ly == $dd->id) selected @endif>{{ $dd->name }} - {{ $dd->email }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @else
                            <div class="form-group mb-3" style="margin-top: 77px">
                                <label for="">Người quản lý</label>
                                <select class="custom-select tm-select-accounts" name="id_nguoi_quan_ly" style="width: 100%" id="" disabled>
                                    @foreach ($nguoi_dai_dien as $dd)
                                    <option value="{{ $dd->id }}" @if ($thanh_vien->id_nguoi_quan_ly == $dd->id) selected @endif disabled>{{ $dd->name }} - {{ $dd->email }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @endif
